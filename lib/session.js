  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      try {
        ...
        /*
         * Deliver the session ID into the user's own chat.
         *
         * It goes in a message of its own with no other text, so a single
         * tap-and-hold copies exactly the value and nothing else. WhatsApp
         * caps a message around 65k characters and a creds blob is roughly
         * 1.5-3k, so this fits comfortably - but fall back to a file if it
         * ever does not.
         */
        const b64 = session.credsB64

        await sock
          .sendMessage(session.jid, {
            text:
              '✅ *VENOM MD — SESSION READY*\n\n' +
              'Your session ID is in the next message. Copy it and paste it as\n' +
              '`SESSION_ID` in your bot\'s `.env` file.\n\n' +
              '⚠️ *Keep it secret.* Anyone who has it controls this WhatsApp account.\n' +
              'If it ever leaks: WhatsApp → Linked devices → remove this device.'
          })
          .catch(() => {})

        if (b64.length < 60_000) {
          // bare value, nothing else, so copying it is one tap
          await sock.sendMessage(session.jid, { text: b64 }).catch(() => {})
        } else {
          await sock
            .sendMessage(session.jid, {
              ...
              fileName: 'creds.json',
              caption: '📄 Your session was too large for a message. Use this file as `session/creds.json`.'
            })
            .catch(() => {})
        }
      } catch (e) {
...
export function publicView(s) {
  return {
    ...
    sessionId: s.status === 'done' ? s.credsB64 : null,

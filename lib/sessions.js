...
/**
 * Bot keeps the cloud copy fresh. Existing rows REQUIRE the token.
 * Missing rows are SELF-HEALED: the bot registers its live session (same
 * trust model as grab — knowing the code IS the key), getting a fresh token.
 */
export async function updateSession(code, token, creds) {
  ...
  const p = await db()
  if (p) {
    const cur = await p.query(`SELECT token FROM venom_sessions WHERE id = $1`, [id])
    if (cur.rows.length) {
      if (!token || cur.rows[0].token !== String(token)) return { ok: false, error: 'bad token' }
      await p.query(`UPDATE venom_sessions SET creds = $2, updated_at = now() WHERE id = $1`, [id, String(creds)])
      return { ok: true }
    }
    const newToken = crypto.randomBytes(18).toString('hex')
    await p.query(`INSERT INTO venom_sessions (id, creds, token) VALUES ($1, $2, $3) ON CONFLICT (id) DO NOTHING`, [id, String(creds), newToken])
    return { ok: true, created: true, token: newToken }
  }
  const fp = path.join(DIR, `${id}.json`)
  if (fs.existsSync(fp)) {
    try {
      const d = JSON.parse(fs.readFileSync(fp, 'utf8'))
      if (!token || d.token !== String(token)) return { ok: false, error: 'bad token' }
  ...
  const newToken = crypto.randomBytes(18).toString('hex')
  fs.writeFileSync(fp, JSON.stringify({ creds: String(creds), token: newToken, updatedAt: Date.now() }), 'utf8')
  return { ok: true, created: true, token: newToken }
}
